...
namespace VChatBox
{
    public partial class VideoBox : MetroFramework.Forms.MetroForm
    {
        ...
        public VideoBox()
        {
            localIP = Profile.myIP;
            localPort = Profile.myPort;

            remoteIP = Profile.friendIP;
            remotePort = Profile.friendPort;
            InitializeComponent();            
        }
        ...
        private void VideoBox_Load(object sender, EventArgs e)
        {
            ...
            Receiver.Start(remotePort.ToString() + "-" + remoteIP);
        ...
        private void Receive(object portip)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    pictureBox1.BackgroundImage = TempImage;
                    LastImageReceived = (Image)pictureBox1.BackgroundImage.Clone();
                    Thread.Sleep(100);
                }
                catch
                {
                    ...
                    {   // in case of error use backup image
                        pictureBox1.BackgroundImage = LastImageReceived;
                    }
                    catch { }
                }
            }
